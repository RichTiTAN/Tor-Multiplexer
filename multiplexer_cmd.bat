...
echo Starting Xray Engine (Hidden)...
powershell.exe -Command "Start-Process -FilePath '%~dp0Data\Xray\xray.exe' -ArgumentList 'run -c config.json' -WorkingDirectory '%~dp0Data\Xray' -WindowStyle Hidden"
...
echo Launching Tor Engine %ID% of %INSTANCE_COUNT% (Hidden)... [ETA: %ETA%s]
