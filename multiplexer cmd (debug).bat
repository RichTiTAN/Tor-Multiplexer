...
cls
echo =========================================
echo    [DEBUG MODE] TOR MULTIPLEXER MENU
echo =========================================
echo.
...
echo   WARNING: 9 WINDOWS WILL POP UP
echo =========================================
echo.

:: Loop through and launch all 8 engines
for /L %%i in (1,1,8) do call :LAUNCH_ENGINE %%i

echo.
echo Starting HAProxy Load Balancer (Visible)...
powershell.exe -Command "Start-Process -FilePath '%~dp0Data\HAproxy\haproxy.exe' -ArgumentList '-f haproxy.cfg' -WorkingDirectory '%~dp0Data\HAproxy' -WindowStyle Normal"
...
cls
echo =========================================
echo   SYSTEMS ACTIVE [%MODE_NAME%] - DEBUG
echo =========================================
echo.
...
set /a "WAIT=ID * 8"
set /a "ETA=72 - WAIT"
echo Launching Tor Engine %ID% (Visible)... [ETA: %ETA%s]

:: Launching Tor visibly for debugging
powershell.exe -Command "Start-Process -FilePath '%~dp0Data\Tors\Tor%ID%\tor.exe' -ArgumentList '-f %TOR_CFG%' -WorkingDirectory '%~dp0Data\Tors\Tor%ID%' -WindowStyle Normal"
